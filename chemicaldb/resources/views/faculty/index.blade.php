...
@section('main-content')
    ...
    @push('modals')
        {{-- @if(\App\Helpers\PermissionHelper::hasFacultyDelete(false)) --}}
    ...
    <div class="col-md-12 table-responsive" style="border: none;">
        @component('layouts.components.table_ajax', ['route'=>'faculty','tname' => 'faculty_table_ajax','bgColor'=>'bg-red']) 

		@slot('url')
            {{ route('faculty.indexAjax')}}
        @endslot 
		...
		@slot('tbody')
            @include('layouts.components.datatable_data_card_render_number',['table_name'=>'faculty_table_ajax']),
            			            @include('layouts.components.datatable_data_card_render', ['table_name'=>'faculty_table_ajax','name'=>'name','title'=>__('faculty.name')]),
		...
		@slot('firstScript')
            {{-- @if(Auth::user()->can('faculty_create')) --}}
