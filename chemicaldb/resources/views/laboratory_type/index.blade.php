...
@section('main-content')
    ...
    @push('modals')
        @if(\App\Helpers\PermissionHelper::hasLaboratoryTypeDelete(false))
    ...
    <div class="col-md-12 table-responsive" style="border: none;">
        @component('layouts.components.table_ajax', ['route'=>'laboratory_type','tname' => 'laboratory_type_table_ajax','bgColor'=>'bg-red']) 

		@slot('url')
            {{ route('laboratory_type.indexAjax')}}
        @endslot 
		...
		@slot('thead')
            <th style='width: 30px;'>{{__('general.number')}}</th>
                        <th>{{__('laboratory_type.name')}}</th>
                        <th style="width:80px;">{{__('general.action')}}</th>
        @endslot 

		@slot('tbody')
            @include('layouts.components.datatable_data_card_render_number',['table_name'=>'laboratory_type_table_ajax']),
            			            @include('layouts.components.datatable_data_card_render', ['table_name'=>'laboratory_type_table_ajax','name'=>'name','title'=>__('laboratory_type.name')]),
		...
		@slot('firstScript')
            @if(Auth::user()->can('laboratory_type_create'))
