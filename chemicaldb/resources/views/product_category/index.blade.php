...
@section('main-content')
    ...
    @push('modals')
        @if(\App\Helpers\PermissionHelper::hasProductCategoryDelete(false))
    ...
    <div class="col-md-12 table-responsive" style="border: none;">
        @component('layouts.components.table_ajax', ['route'=>'product_category','tname' => 'product_category_table_ajax','bgColor'=>'bg-red']) 

		@slot('url')
            {{ route('product_category.indexAjax')}}
        @endslot 
		...
		@slot('thead')
            <th style='width: 30px;'>{{__('general.number')}}</th>
                        <th>{{__('product_category.name')}}</th>
                        <th style="width:80px;">{{__('general.action')}}</th>
        @endslot 

		@slot('tbody')
            @include('layouts.components.datatable_data_card_render_number',['table_name'=>'product_category_table_ajax']),
            			            @include('layouts.components.datatable_data_card_render', ['table_name'=>'product_category_table_ajax','name'=>'name','title'=>__('product_category.name')]),
