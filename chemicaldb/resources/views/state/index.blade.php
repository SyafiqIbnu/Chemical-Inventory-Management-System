...
@section('main-content')
    ...
    @push('modals')
        @if(\App\Helpers\PermissionHelper::hasStateDelete(false))
    ...
    <div class="col-md-12 table-responsive" style="border: none;">
        @component('layouts.components.table_ajax', ['route'=>'state','tname' => 'state_table_ajax','bgColor'=>'bg-red']) 

		@slot('url')
            {{ route('state.indexAjax')}}
        @endslot 
		...
		@slot('tbody')
            @include('layouts.components.datatable_data_card_render_number',['table_name'=>'state_table_ajax']),
            			            @include('layouts.components.datatable_data_card_render', ['table_name'=>'state_table_ajax','name'=>'name','title'=>__('state.name')]),
